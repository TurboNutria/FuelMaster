...
class MapViewController: UIViewController, MKMapViewDelegate, StationDetialDelegate, MapFilterViewDelegate, CLLocationManagerDelegate, UIGestureRecognizerDelegate {
    ...
    func refreshArea() {
            ...
            if let region = UserDefaults.standard.value(forKey: "region") as? String {
                let area = GeocoderManager.getRegion(lat: self.map.centerCoordinate.latitude, lon: self.map.centerCoordinate.longitude)
                if area != region {
    ...
    func locationManager(_ manager: CLLocationManager, didChangeAuthorization status: CLAuthorizationStatus) {
        if status == .authorizedWhenInUse  || status == .authorizedAlways   {
            ...
            currentRegion = GeocoderManager.getRegion(lat: (locationManager?.location?.coordinate.latitude)!, lon: (locationManager?.location?.coordinate.longitude)!)
